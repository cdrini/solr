...
The `defType` parameter selects the xref:query-syntax-and-parsers.adoc[query parser] to process the main query parameter (`q`) in the request. Acceptable values are: `lucene` (xref:standard-query-parser.adoc[]), `dismax` (xref:dismax-query-parser.adoc[]), or `edismax` (xref:edismax-query-parser.adoc[]).
...
* Multi-valued primitive fields or SortableTextFields with `docValues="true"`. In this case, a representative value is chosen for each document depending on the sort direction. When ascending, the minimum value is used. When descending, the maximum value is used. This is equivalent to explicitly sorting using the 2 argument xref:function-queries.adoc#field-function[`field()`] function: `sort=field(name,min) asc` and `sort=field(name,max) desc`.

Multiple sort orderings can be specified by separating them with commas, e.g. `inStock desc,price asc`.
In this case, the second entry will only be used if the first entry results
in a tie. If there is a third entry, it will only be used if the first _and_ second entries are tied, and
so on.
...
[%autowidth,frame=none]
|===
|Optional |Default: an autogenerated UUID
|===
...
It is also very useful for speeding up complex queries, since the queries specified with `fq` are cached independently of the main query.
This means that if a later query uses the same filter, filter results are returned quickly from the cache.

If multiple `fq` parameters are specified, documents must match all the `fq` parameters.
For example, in the query `fq=popularity:[10 TO *]&fq=section:0`, only documents which have a popularity greater than 10 and have a section of 0 will match.

The document sets from each filter are cached independently. For example, although the parameters `fq=popularity:[10 TO *]&fq=section:0` and `fq=+popularity:[10 TO *] +section:0` both return the same results, the first creates two caches, whereas the second creates only one. So as a guideline, use a single `fq` parameter with multiple clauses if those clauses appear together often, and use two separate `fq` parameters if they are relatively independent.
...
NOTE: As with all parameters: special characters in a URL need to be properly escaped and encoded as hex values.
...
You can disable this behaviour using the `cache` xref:local-params.adoc[local param], e.g. `fq={!cache=false}section:0`, if you think a query is unlikely to be repeated.
...
The `fl` parameter is a comma- or space-separated list that limits the fields included in a query response.

Here are some example `fl` parameters:

[%autowidth.stretch,options="header"]
|===
|Field List |Result
|`id,title,price` |Returns only the `id`, `name`, and `price` fields.
|`id,link_*` |Returns the `id` field and any field starting with `link_`.
|`id,score` |Returns the `id` field and the document score.
|`*` |Returns all the `stored` fields in each document, as well as any `docValues` fields that have `useDocValuesAsStored="true"`. This is the default value of the `fl` parameter.
|`*,score` |Returns `score` in addition to all the fields of the document.
|`*,dv_field_name` |Returns all the `stored` fields in each document, and any `docValues` fields that have `useDocValuesAsStored="true"` and the docValues from dv_field_name even if it has `useDocValuesAsStored="false"`
|===

More specifically, a returned field can be:

* A field in the document that is `stored="true"` or `docValues="true"`
** E.g. `id,title,price`.
* The pseudo-field `score`, indicating the document score.
* The wildcard character `*`, indicating all fields in the document. Note that fields must be either `stored="true"` or `docValues="true"` and `useDocValuesAsStored="true"` (which is the default when docValues are enabled).
** E.g. `*` returns all the document fields.
** E.g. `*,score` returns `score` in addition to all the document fields.
* A field name with the wildcard character, indicating a glob pattern for matching multiple field names.
** E.g. `id,link_*` returns `id` and all fields beginning with `link_`.
* A xref:function-queries.adoc[Function Query]
** E.g. `id,title,product(price,popularity)` returns a field called `product(price,popularity)` containing the result of the function.
* A xref:document-transformers.adoc[Document Transformer]
** e.g. `id,title,[explain]` returns a field called `[explain]` containing the result of the xref:document-transformers.adoc#explain-explainaugmenterfactory[explain transformer].
* Any of the above, with an alias.
** E.g. `id,secret_sauce:prod(price,popularity)` returns the results of the function query in a field called `secret_sauce`.

For a more complete example, the `fl` parameter `id,sales_price:price,secret_sauce:prod(price,popularity),why_score:[explain style=nl]` would return a document like:
